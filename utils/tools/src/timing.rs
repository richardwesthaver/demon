...
pub struct Time {
  ...
  delta_time: Duration,
  /// Time elapsed since the last frame in seconds ignoring the time speed
  /// multiplier.
  ...
  ///Time multiplier. Affects returned delta_seconds, delta_time and
  /// absolute_time.
  time_scale: f32,
...
impl Time {
  ...
  ///
  /// This function should not be used during `fixed_update`s, use
  /// `fixed_seconds` instead.
  ...
  /// Gets the time difference between frames in seconds ignoring the time
  /// speed multiplier.
  ///
  ...
  /// Gets the time difference between frames ignoring the time speed
  /// multiplier.
  ...
  /// Gets the current frame number.  This increments by 1 every frame.  There
  /// is no frame 0.
  pub fn frame_number(&self) -> u64 {
    self.frame_number
  }

  /// Gets the time since the start of the game, taking into account the speed
  /// multiplier.
  pub fn absolute_time(&self) -> Duration {
    self.absolute_time
  }

  /// Gets the time since the start of the game as seconds, taking into
  /// account the speed multiplier.
  pub fn absolute_time_seconds(&self) -> f64 {
    duration_to_secs_f64(self.absolute_time)
  }

  /// Gets the time since the start of the game, ignoring the speed
  /// multiplier.
  pub fn absolute_real_time(&self) -> Duration {
    self.absolute_real_time
  }

  /// Gets the time since the start of the game as seconds, ignoring the speed
  /// multiplier.
  ...
  ///
  /// This should only be called by the engine.  Bad things might happen if
  ...
  /// Restarts the internal fixed update accumulator to the desired fixed
  /// update delta time.
  ///
  /// This should only be called by the engine.  Bad things might happen if
  /// you call this in your game.
  ...
  /// Checks to see if we should perform another fixed update iteration, and
  /// if so, returns true and reduces the accumulator.
  ///
  /// This should only be called by the engine.  Bad things might happen if
  /// you call this in your game.
  ...
  /// Updates the interpolation alpha factor given the current fixed update
  /// rate and accumulator.
  ///
  /// This should only be called by the engine.  Bad things might happen if
  /// you call this in your game.
...
mod tests {
  ...
  // Test confirms that with a fixed update of 120fps, we run fixed update twice
  // with the timer Runs at 10 times game speed, which shouldn't affect fixed
  // updates
  #[test]
