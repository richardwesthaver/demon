...
use tools::net::{display_connection_string, display_ip_or_host};
use tui::{
  backend::Backend,
...
use crate::app::{Bandwidth, DisplayBandwidth, UiState};
...
fn truncate_middle(row: &str, max_length: u16) -> String {
  if max_length < 6 {
    ...
    let second_slice = truncate_iter_to_unicode_width::<_, Vec<_>>(row.chars().rev(), split_point)
      .into_iter()
      .rev()
      .collect::<String>();
    if max_length % 2 == 0 {
...
impl<'a> Table<'a> {
  ...
  pub fn render(&self, frame: &mut Frame<impl Backend>, rect: Rect) {
    ...
    let column_names = match column_count {
      ColumnCount::Two => {
        vec![self.column_names[0], self.column_names[2]] // always lose
                                                         // the
                                                         // middle column when
                                                         // needed
      }
